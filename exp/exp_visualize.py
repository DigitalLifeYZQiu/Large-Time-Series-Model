...
class Exp_Visualize(Exp_Basic):
    ...
    def test(self, setting, test=0):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    ...
                    for j in range(inference_steps):
                        ...
                        if self.args.output_attention:
                            ...
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                            # record embeddings
                            embed = self.model.getEmbedding(batch_x).detach().cpu()
                            import pdb
                            pdb.set_trace()
                    ...
                    # concat embeddings over inference steps
        ...
        import pdb
        pdb.set_trace()
        embeds = torch.cat(embeddings_list, dim=0)

        # reshape
        embeds = embeds.reshape(embeds.shape[0],-1)
        from sklearn.manifold import TSNE
        # tsne = TSNE(n_components=2, verbose=1)
        # tsne_results = tsne.fit_transform(embeds[:5000])

        from utils.tsne import visualization
        label = torch.zeros(5000)
        token_nums = 1
        visualization(embeds[:5000],label,token_nums, './vis', 'test')
